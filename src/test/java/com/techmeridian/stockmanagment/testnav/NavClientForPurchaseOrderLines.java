...
public class NavClientForPurchaseOrderLines {

	public static void main(String[] args) {
		try {
			SoapObject response = ConnectToNAVCodeUnit("urn:microsoft-dynamics-schemas/page/purchaselineslist",
					"http://122.166.222.116:50047/DynNAV/WS/CRONUS%20India%20Ltd./Page/PurchaseLinesList",
					"urn:microsoft-dynamics-schemas/page/purchaselineslist:ReadMultiple", "ReadMultiple");

			System.out.println("***************************************");
			for (int j = 0; j < response.getPropertyCount(); j++) {
				SoapObject pii = (SoapObject) response.getProperty(j);
				
				System.out.println(getValue("Document_No", pii));
	...
	public static SoapObject ConnectToNAVCodeUnit(String namespace, String url, String soap_action,
		...
		try {
			...
			ntlm.debug = false;
			ntlm.setCredentials(url, "administrator", "itreenav", "KAGEM", "");
