...
public class NtlmTransport extends Transport {
	...
	public List call(String soapAction, SoapEnvelope envelope, List headers, File outputFile)
		...
		try {
			...
			System.out.println("call: status_code" + status_code);
	...
	// Try to execute a cheap method first. This will trigger NTLM
	// authentication
	public void setupNtlm(String dummyUrl, String userId, String password) {
		try {
			...
			System.out.println("setupNtlm: response: " + response.getStatusLine().getStatusCode());
			HttpEntity entity1 = response.getEntity();

			Header[] hArray = response.getAllHeaders();
			...
			for (int i = 0; i < size; i++) {
				Header h = hArray[i];
				System.out.println("setupNtlm: h.getName()" + h.getName());
